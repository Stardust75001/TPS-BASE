...
Shopify.formatMoney = function (cents, format) {
  try {
    ...
    // Ensure numeric cents
    const nCents = Number(cents);
    let value = "";
    const placeholderRegex = /\{\{\s*(\w+)\s*\}\}/;

    // Avoid relying on `this` which may be undefined in strict mode
    let formatString =
      typeof format === "string"
        ? format
        : typeof window.Shopify?.money_format === "string"
        ? window.Shopify.money_format
        : "{{ amount }}";

    if (
      typeof formatString !== "string" ||
      !placeholderRegex.test(formatString)
    ) {
      formatString = "{{ amount }}";
    }

    function d(o, defVal) {
      return typeof o === "undefined" ? defVal : o;
    }
    ...
    function f(n, p, t, dSep) {
      ...
      const fixed = (n / 100).toFixed(p);
      const parts = fixed.split(".");
      const dollars = parts[0].replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1" + t);
      const centsPart = parts[1] ? dSep + parts[1] : "";
      return dollars + centsPart;
    }

    const m = formatString.match(placeholderRegex);
    const token = m && m[1] ? m[1] : "amount";
    switch (token) {
      case "amount":
        value = f(nCents, d, undefined, undefined);
        break;
      case "amount_no_decimals":
        value = f(nCents, 0, undefined, undefined);
        break;
    ...
    // Fail-safe: never break rendering due to money formatting
    try {
